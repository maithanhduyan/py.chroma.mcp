...
async def semantic_chunking_by_embedding(
    ...
    # Chia text thành sentences dựa trên dấu câu
    ...
    # Sử dụng embedding function mặc định của ChromaDB
    embedding_func = DefaultEmbeddingFunction()

    try:
        # Tạo embeddings cho tất cả sentences
        embeddings = embedding_func(sentences)
        embeddings = np.array(embeddings)

        # Khởi tạo chunks với sentence đầu tiên
        chunks = []
        current_chunk_sentences = [sentences[0]]
        current_chunk_embeddings = [embeddings[0]]

        # Duyệt qua từng sentence và quyết định có thêm vào chunk hiện tại không
        for i in range(1, len(sentences)):
            current_sentence = sentences[i]
            current_embedding = embeddings[i]

            # Tính similarity với chunk hiện tại (average embedding)
            chunk_avg_embedding = np.mean(current_chunk_embeddings, axis=0).reshape(
                1, -1
            )
            sentence_embedding = current_embedding.reshape(1, -1)

            similarity = cosine_similarity(chunk_avg_embedding, sentence_embedding)[0][
                0
            ]

            # Nếu similarity >= threshold, thêm vào chunk hiện tại
            if similarity >= similarity_threshold:
                current_chunk_sentences.append(current_sentence)
                current_chunk_embeddings.append(current_embedding)
            else:
                # Ngược lại, tạo chunk mới
        ...
        raise Exception(f"Failed to perform semantic chunking: {str(e)}") from e
